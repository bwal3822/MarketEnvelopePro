...
data_df = pd.read_excel('/mnt/data/MES2022Algo.xlsx', sheet_name='Data').iloc[2:].reset_index(drop=True)
...
data_df[['Open', 'High', 'Low', 'Close']] = data_df[['Open', 'High', 'Low', 'Close']].astype(float)
...
data_df['Direction'] = data_df['Close'].diff().apply(lambda x: 'Up' if x > 0 else ('Down' if x < 0 else 'Sideways'))

# Identify Buy, Sell, and Short Days (assuming the cycle starts at the first row, i.e., index 0)
